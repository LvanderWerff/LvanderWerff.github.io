 ...
 * generated on 2015-12-02T08:25:35+01:00
 */
/*! datawrapper - v1.10.0 */
...
if(lpos){lbl.animate({x:lpos.left,y:lpos.top,align:lpos.halign,valign:lpos.valign},theme.duration,theme.easing);if(!lbl.hasClass('value')){if(lpos.valign=="bottom"){$(lbl.el).addClass("lbl-align-left").removeClass("lbl-align-right");}else if(lpos.valign=="top"){$(lbl.el).addClass("lbl-align-right").removeClass("lbl-align-left");}}
if(lbl.hasClass('value')){var barDimensions=me.barDimensions(bar,s);if(lpos.width>(barDimensions.width+barDimensions.width*barDimensions.pad)){lbl.addClass("showOnHover");lbl.hide();}
else{lbl.removeClass("showOnHover");lbl.show()}}}});updated_bars[bar.name]=true;});_.each(me.__elements,function(elements,key){if(!updated_bars[key]){me.__barExit(key);}});if(me.__gridLines['0'])me.__gridLines['0'].toFront();},getBarColor:function(bar,useNegativeColor,colorful){var me=this;return me.getKeyColor(bar?bar.name:null,bar?bar.value:null,useNegativeColor);},initDimensions:function(r){var me=this,c=me.__canvas,domain=me.getBarColumn().range();if(me.get('absolute-scale',false)){domain=dw.utils.minMax(_.map(me.axesDef.columns,function(c){return me.dataset.column(c);}));}
